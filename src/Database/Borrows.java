...
@NamedQueries({
    ...
    @NamedQuery(name = "Borrows.findAllWithMonthYear", query = "SELECT b FROM Borrows b WHERE EXTRACT(MONTH FROM b.createdAt) = :month AND EXTRACT(YEAR FROM b.createdAt) = :year"),
    ...
    @NamedQuery(name = "Borrows.findByLikeThesisJudulWithMonthYear", query = "SELECT b FROM Borrows b WHERE LOWER(b.tesisId.judul) LIKE LOWER(:parameter) AND EXTRACT(MONTH FROM b.createdAt) = :month AND EXTRACT(YEAR FROM b.createdAt) = :year"),
